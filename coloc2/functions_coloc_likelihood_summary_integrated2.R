...
process.dataset <- function(d, suffix, ave = TRUE, estimate_sdy = TRUE, correlation = 0) {
  ...
  if ("pvalues" %in% nd & "MAF" %in% nd & "N" %in% nd) {
    if (length(d$pvalues) != length(d$MAF)) {
      message(paste0("length(d$pvalues): ", length(d$pvalues)))
      message(paste0("length(d$MAF): ", length(d$MAF)))
...
coloc.abf <- function(dataset1, dataset2, MAF = NULL,
  ...
  if ((!("MAF" %in% names(dataset1)) | is.null(dataset1$MAF)) & !is.null(MAF)) {
    dataset1$MAF <- MAF
  }
  if ((!("MAF" %in% names(dataset2)) | is.null(dataset2$MAF)) & !is.null(MAF)) {
    dataset2$MAF <- MAF
  }
...
coloc.eqtl.biom <- function(eqtl.df, biom.df, p12 = 1e-6, useBETA = TRUE, plot = FALSE, outfolder, prefix = "pref", save.coloc.output = FALSE, match_snpid = TRUE, cores = 20, bootstrap = F, no_bootstraps = 1000, min_snps = 50, bed_input_file = NULL) {
  ...
  # source("/home/fanwang/grlasso/simu47/optim_function.R")
  # source("optim_function.R")
  # source("/hpf/largeprojects/struglis/cfcentre/strug/fanwang/grlasso/simu53/optim_function.R")
  # print(head(eqtl.df))
  # print(head(biom.df))
  ...
  if (!all(cols.eqtl %in% names(eqtl.df))) {
    if (!all(cols.eqtl[-which(cols.eqtl %in% c("A1", "A2"))] %in% names(eqtl.df))) {
      ...
      stop(paste0("These columns are missing from the eQTL data: ", paste(cols.eqtl[!cols.eqtl %in% names(eqtl.df)], collapse = ", ")))
    } else {
      no_allele_merge = T
  ...
  for (i in 1:length(list.probes)) {
    ...
    toremove = merged.data$PVAL.biom <= 0 | merged.data$PVAL.eqtl <= 0 | is.na(merged.data$PVAL.biom) | is.na(merged.data$PVAL.eqtl)
    removed_snp_list = data.frame()
    if (sum(toremove) > 0) {
      removed_snp_list = rbind(removed_snp_list, data.frame(ProbeID = merged.data$ProbeID[toremove], Marker_removed = merged.data$SNP[toremove], reason = "PVALs zero or NA"))
    ...
    if (nsnps <= min_snps) {
      ...
      for (i in 1:length(split_merged_data)) {
        ...
        if (!useBETA) {
          ## here I change the dataset.biom type=type to type=quant
          dataset.biom = list(
            snp = merged.data$SNPID, pvalues = merged.data$PVAL.biom,
            N = merged.data$N.biom, s = merged.data$s1, type = "quant", MAF = merged.data$MAF.biom
          )
          dataset.eqtl = list(
            snp = merged.data$SNPID, pvalues = merged.data$PVAL.eqtl,
            N = merged.data$N.eqtl, type = "quant", MAF = merged.data$MAF.eqtl
          )
        } else {
          dataset.biom = list(
            snp = merged.data$SNPID, beta = merged.data$BETA.biom, varbeta = (merged.data$SE.biom)^2,
            s = merged.data$s1, type = "quant", MAF = merged.data$MAF.biom, N = merged.data$N.biom, sdY = unique(merged.data$sdY.biom)
          )
          dataset.eqtl = list(
            snp = merged.data$SNPID, beta = merged.data$BETA.eqtl, varbeta = (merged.data$SE.eqtl)^2,
            N = as.numeric(merged.data$N.eqtl), type = "quant", MAF = merged.data$MAF.eqtl
          )
        }
        MAF = NULL
        if (maf.eqtl) {
          MAF = merged.data$MAF.eqtl
        } else {
          MAF = merged.data$MAF.biom
        }

        (coloc.res <- coloc.abf(dataset.biom, dataset.eqtl, MAF = MAF, p12 = p12))
        ...
        if (plot & (pp4 > 0.2 | pp3 >= 0.2) & nsnps > 2 & F) {
          ...
          plotted = tryCatch(locuszoom.ms(
            metal = pvalue_BF_file,
  ...
  if (bootstrap) {
    ...
    cis = t(apply(bootstrap.all, 2, function(x) {
  ...
  for (i in 1:length(list.probes)) {
    ...
    if (nsnps <= min_snps) {
      ...
      for (i in 1:length(split_merged_data)) {
        ...
        if (!useBETA) {
          ## here I changed type=type to type=quant
          dataset.biom = list(
            snp = merged.data$SNPID, pvalues = merged.data$PVAL.biom,
            N = merged.data$N.biom, s = merged.data$s1, type = "quant", MAF = merged.data$MAF.biom
          )
